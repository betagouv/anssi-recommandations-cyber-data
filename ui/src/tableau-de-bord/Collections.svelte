        ...
        >
          <h2 class="text-lg font-bold text-gray-900 mb-2">
            ...
            <span class="text-sm font-normal text-gray-500"
              >(id : {collection.id})</span
            >
          </h2>
          <p class="text-gray-600 mb-4 flex-grow">{collection.description}</p>
          <p class="text-gray-900 mb-4">
            <strong class="font-bold">{collection.nombre_documents}</strong> documents
          </p>
          <div
            class="mt-auto pt-4 border-t border-gray-100 text-xs text-gray-500 space-y-1"
          >
            <p>Créé le : {formaterDate(collection.date_de_creation)}</p>
            <p>
              Dernière modification : {formaterDate(
                collection.date_de_derniere_modification
              )}
            </p>
          </div>
        </section>
