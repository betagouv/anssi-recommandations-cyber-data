  ...
  const authentifie = async () => {
    ...
    const authenticationResponse = await startAuthentication({
      optionsJSON: {
        ...
        allowCredentials: [{ id: initialisation.id, type: 'public-key' }],
      },
    });
    ...
    const reponseFinalisation = await fetch('/auth/finalise', {
      ...
      body: JSON.stringify({
        ...
        utilisateur: identifiantAuthentification,
      }),
    });
    ...
    console.log(
      `CONNECTÉ ? ${reponseFinalisation.status} : ${await reponseFinalisation.text()} - ${await reponseFinalisation.json()}`
    );
  };
...
<main class="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
  <div class="max-w-4xl mx-auto">
    <header class="mb-8">
      <h3 class="text-3xl font-bold text-gray-900 border-b pb-4">
        Authentification
      </h3>
    </header>
          ...
          <div>
            <section class="grid gap-4">
              <div class="space-y-6">
                <input
                  type="text"
                  bind:value={identifiantEnrolement}
          ...
          <div>
            <section class="grid gap-4">
              <div class="space-y-6">
                <input
                  type="text"
                  bind:value={identifiantAuthentification}
                  class="w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                />
              </div>
              <div>
