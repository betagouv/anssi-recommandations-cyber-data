...
from src.main_evalap import applique_mapping_noms_documents, prepare_dataframe
...
def test_applique_mapping_noms_documents_avec_ref_valide(tmp_path: Path):
    ...
    assert "nom_document_reponse_bot" in df_resultat.columns
    assert "nom_document_verite_terrain" in df_resultat.columns
    assert df_resultat["nom_document_reponse_bot"].iloc[0] == "Doc1"
    assert df_resultat["nom_document_verite_terrain"].iloc[0] == "guide-auth.pdf"
    assert df_resultat["nom_document_reponse_bot"].iloc[1] == "Doc3"
...
def test_applique_mapping_noms_documents_avec_liste_vide():
    ...
    mapping_csv = Path("./donnees/QA-labelisé-Mapping_Nom.csv")
    df_resultat = applique_mapping_noms_documents(df, mapping_csv)

    assert df_resultat["nom_document_reponse_bot"].iloc[0] == ""
