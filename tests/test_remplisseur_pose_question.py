...
@respx.mock
def test_remplissage_ajoute_colonne_numeros_page_liste_tous_paragraphes(
    tmp_path: Path, configuration: MQC
):
    fichier = tmp_path / "eval.csv"
    fichier.write_text("Question type\nQ1?\n", encoding="utf-8")
    ...
    assert "Numéros Page" in df.columns
    assert df["Numéros Page"].iloc[0] == [5, 12]


@respx.mock
def test_remplissage_ajoute_colonne_numeros_page_liste_vide_si_aucun_paragraphe(
    tmp_path: Path, configuration: MQC
):
    fichier = tmp_path / "eval.csv"
    fichier.write_text("Question type\nQ1?\n", encoding="utf-8")
